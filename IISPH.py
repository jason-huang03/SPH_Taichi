...
class IISPHSolver(SPHBase):
    def __init__(self, particle_system):
        ...
        self.ps.particle_accelerations = ti.Vector.field(self.ps.dim, dtype=float)
        self.pressure_accel = ti.Vector.field(self.ps.dim, dtype=float)
        particle_node = ti.root.dense(ti.i, self.ps.particle_max_num)
        particle_node.place(self.ps.particle_accelerations, self.pressure_accel)
        self.dt[None] = 2e-4
    ...
    def predict_advection(self):
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            m_Vi = self.ps.particle_volumes[p_i]
            density_i = self.ps.particle_densities[p_i]
            ...
            for j in range(self.ps.fluid_neighbors_num[p_i]):
                ...
                for k in range(self.ps.fluid_neighbors_num[p_i]):
                    density_k = self.ps.particle_densities[k]
                    density_k2 = density_k * density_k
                    p_k = self.ps.fluid_neighbors[p_i, j]
                    x_k = self.ps.particle_positions[p_k]
                    sum_neighbor_inner += self.ps.particle_volumes[p_k] * self.cubic_kernel_derivative(x_i - x_k) / density_k2

                kernel_grad_ij = self.cubic_kernel_derivative(x_i - x_j)
                sum_neighbor -= (self.ps.particle_volumes[p_j] * sum_neighbor_inner).dot(kernel_grad_ij)

                sum_neighbor_of_neighbor -= (self.ps.particle_volumes[p_j] * kernel_grad_ij).dot(kernel_grad_ij)
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            for j in range(self.ps.fluid_neighbors_num[p_i]):
                p_j = self.ps.fluid_neighbors[p_i, j]
                x_j = self.ps.particle_positions[p_j]
                divergence += self.ps.particle_volumes[p_j] * (self.ps.particle_velocities[p_i] - self.ps.particle_velocities[p_j]).dot(self.cubic_kernel_derivative(x_i - x_j))
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            self.last_pressure[p_i] = 0.5 * self.ps.particle_pressures[p_i]
    ...
    def pressure_solve_iteration(self):
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            dpi = self.last_pressure[p_i] / self.ps.particle_densities[p_i] ** 2
            # Fluid neighbors
            for j in range(self.ps.fluid_neighbors_num[p_i]):
                p_j = self.ps.fluid_neighbors[p_i, j]
                x_j = self.ps.particle_positions[p_j]
                dpj = self.last_pressure[p_j] / self.ps.particle_densities[p_j] ** 2
                # Compute the pressure force contribution, Symmetric Formula
                d_v += -self.density_0 * self.ps.particle_volumes[p_j] * (dpi + dpj) \
        ...
        for p_i in range(self.ps.particle_num[None]):
            ...
            for j in range(self.ps.fluid_neighbors_num[p_i]):
                p_j = self.ps.fluid_neighbors[p_i, j]
                x_j = self.ps.particle_positions[p_j]
                Ap += self.ps.particle_volumes[p_j] * (accel_p_i - self.pressure_accel[p_j]).dot(self.cubic_kernel_derivative(x_i - x_j))
            # Boundary neighbors
            ## Akinci2012
            for j in range(self.ps.solid_neighbors_num[p_i]):
                p_j = self.ps.solid_neighbors[p_i, j]
            ...
            if abs(self.a_ii[p_i]) > 1e-6:
                # Relaxed Jacobi
                self.ps.particle_pressures[p_i] = ti.max(self.last_pressure[p_i] + omega * (self.density_deviation[p_i] - Ap) / self.a_ii[p_i], 0.0)
            else:
                self.ps.particle_pressures[p_i] = 0.0

            if self.ps.particle_pressures[p_i] != 0.0:
        ...
        for p_i in range(self.ps.particle_num[None]):
            # Update the pressure
            self.last_pressure[p_i] = self.ps.particle_pressures[p_i]
    ...
    def compute_densities(self):
        for p_i in range(self.ps.particle_num[None]):
            ...
            for j in range(self.ps.fluid_neighbors_num[p_i]):
                p_j = self.ps.fluid_neighbors[p_i, j]
                x_j = self.ps.particle_positions[p_j]
                self.ps.particle_densities[p_i] += self.ps.particle_volumes[p_j] * self.cubic_kernel((x_i - x_j).norm())
            # Boundary neighbors
            ## Akinci2012
            for j in range(self.ps.solid_neighbors_num[p_i]):
                p_j = self.ps.solid_neighbors[p_i, j]
    ...
    def compute_pressure_forces(self):
        for p_i in range(self.ps.particle_num[None]):
            ...
            for j in range(self.ps.fluid_neighbors_num[p_i]):
                p_j = self.ps.fluid_neighbors[p_i, j]
                x_j = self.ps.particle_positions[p_j]
                dpj = self.ps.particle_pressures[p_j] / self.ps.particle_densities[p_j] ** 2
                # Compute the pressure force contribution, Symmetric Formula
                d_v += -self.density_0 * self.ps.particle_volumes[p_j] * (dpi + dpj) \
                       * self.cubic_kernel_derivative(x_i - x_j)

            # Boundary neighbors
            dpj = self.ps.particle_pressures[p_i] / self.density_0 ** 2
            # dpj = 0.0
            ## Akinci2012
            for j in range(self.ps.solid_neighbors_num[p_i]):
                p_j = self.ps.solid_neighbors[p_i, j]
    ...
    def advect(self):
        ...
        for p_i in range(self.ps.particle_num[None]):
            if self.ps.particle_materials[p_i] == self.ps.material_fluid:
                self.ps.particle_velocities[p_i] += self.dt[None] * self.pressure_accel[p_i]
